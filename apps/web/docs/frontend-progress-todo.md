...
| 用户画像 | ready | doing | `/profile` 已重组为个人中心仪表盘，补上阶段主任务、关键日程、周视图和简历状态区 | 继续补趋势分析、更细的身份信息编排和跨页导流 |
...
| 岗位定向改写建议 | ready | doing | 同步接口仍可用，异步任务版接口已上线，适合后续接入对话工作区和长任务状态提示 | 评估页面先接同步还是异步，并补任务态交互 |
| AI 异步任务 | ready | todo | 后端已提供创建任务、查状态/结果、WebSocket 通知，第一批接入点是岗位定向改写建议 | 评估先在哪个页面接入任务态体验 |
...
- [x] 个人中心先完成 V2 仪表盘化改造，把已有画像能力组织成用户阶段页
- [x] 首页真实数据切换时，明确提示当前是 `live` 还是 `demo`
- [ ] AI 页面统一处理 `401` / `404` / `503`
- [ ] 为岗位改写建议评估是否切到异步任务接口，并明确创建中 / 处理中 / 已完成状态
...
- `POST /api/jobs/:id/resume/rewrite-suggestions/tasks`
  - 当前为异步任务版入口，适合对话式或长耗时工作区
  - 需要配套接 `GET /api/ai/tasks/:id` 或 `GET /api/ai/tasks/ws`
  - 当前第一版依然建议结果页最终以任务详情为权威来源
