...
*   黄色阶段任务卡、深色对话舞台、周视图时间板、彩色企业推荐卡等核心视觉语言已创建组件，但尚未在各页面内系统性替换旧区块。
*   首页 `/` 仍使用旧版 `page-stack` 内容布局（`home-hero` / `metrics` / `overview` 等旧 section），尚未重排为”左辅助栏 + 中部主任务 + 右知识树”结构。
*   个人中心 `/profile` 仍为旧版表单页布局，尚未升级为完整个人中心仪表盘。
*   AI 对话工作区尚未以正式产品页实现（组件就绪，路由 `/advisor-chat` 待 Phase 4）。
*   面试模拟页尚未实现（路由 `/interview` 待 Phase 4）。
...
*   当前 `/` 已接入：
    ...
    *   小型时间线预览
*   但仍未对齐：
    *   中部内容区仍为旧版 `page-stack` 布局（`home-hero` / `metrics` / `overview` 等旧 section）
    *   尚未重排为"左辅助栏 + 中部主任务 + 右知识树"的正式产品骨架
    *   引导式主任务卡、精准推送岗位横卡等尚未接入中部主舞台
...
*   仍未完成：
    *   图稿里的“个人中心”一体化仪表盘页面
    *   关键日程、简历管理、GPA/趋势、人格特质、当前主任务、周视图日程的统一承载
